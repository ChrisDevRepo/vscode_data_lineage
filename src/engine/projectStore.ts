...
/**
 * Extracts the offending property names from a strict-mode ("unrecognized keys") Zod issue.
 *
 * @remarks
 * Zod anchors that issue at the *container* path and lists the rejected names in `keys`, so
 * without this the report would name the object and never the field that caused the drop —
 * which is the one thing a user needs to know when a saved project disappears.
 *
 * @param issue - A Zod issue of unknown shape.
 * @returns The rejected key names, or an empty array for any other issue code.
 */
function unrecognizedKeysOf(issue: unknown): string[] {
  const candidate = issue as { code?: unknown; keys?: unknown };
  if (candidate.code !== 'unrecognized_keys' || !Array.isArray(candidate.keys)) return [];
  return candidate.keys.map((key) => String(key));
}
...
export function migrateProjectStore(
  ...
  const projects = (obj.projects as unknown[]).flatMap((project) => {
    ...
    for (const issue of parsed.error.issues) {
      ...
      const rejectedKeys = unrecognizedKeysOf(issue);
      const paths = rejectedKeys.length > 0
        ? rejectedKeys.map((key) => (base ? `${base}.${key}` : key))
        : [base || '(root)'];
