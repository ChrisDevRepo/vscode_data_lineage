...
export function minifyDdlForHop(raw: string, preserveTechContext: boolean): string {
  ...
  clean = clean.replace(/^[\t ]*USE\s+\[?[\w\s_]+\]?[\t ]*;?[\t ]*$/gmi, '');
  // 3. SET statements
  ...
  if (!preserveTechContext) {
    ...
    clean = clean.replace(/\bWITH\s*\([\s\S]*?(PAD_INDEX|FILLFACTOR|STATISTICS_NORECOMPUTE|IGNORE_DUP_KEY|ALLOW_ROW_LOCKS|ALLOW_PAGE_LOCKS|OPTIMIZE_FOR_SEQUENTIAL_KEY)[\s\S]*?\)/gi, '');
