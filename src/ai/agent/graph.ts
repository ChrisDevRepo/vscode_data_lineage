...
import { formatProviderErrorDiagnostic, type ProviderErrorDiagnostic } from '../support/text';
import {
 ...
 *
 * @remarks
 * Counted from the graph wiring: the three fixed non-looping nodes `consent_gate`, `approve_gate` and
 * `cancel_gate` (3), the extra `active_coordinator` execution that routes to `synthesis` after the last
 * round (1), and the START/END plumbing transitions (2). A small constant, not a policy knob.
 */
const FIXED_TRANSITION_OVERHEAD = 6;
...
// Exported so pinning tests can assert on the production string directly rather than duplicating it.
 ...
 * {@link MAX_TOOL_PROVIDER_CALLS}, so the real transition count is far above the old flat per-round
...
export function buildAgentGraph(deps: AgentGraphDeps) {
  ...
  /** Executes one compiled tool plan against the phase-local cumulative budget. */
  ...
  const detectEntryNode = async (state: AgentStateType): Promise<AgentStateUpdate> => {
    ...
    // Depth is now entirely AI-owned through the lineage_start_exploration tool payload.
    // The entry detector no longer attempts to extract depth upfront.
  ...
  const activeCoordinatorNode = (state: AgentStateType): AgentStateUpdate => {
    ...
    deps.sink.status('thinking', 'Analysing hop-by-hop...');
    const sess = deps.getSession();
  ...
  const synthesisNode = async (state: AgentStateType): Promise<AgentStateUpdate> => {
    ...
    if (sess.lastPresentResultSummary) {
  ...
  const followUpNode = async (state: AgentStateType): Promise<AgentStateUpdate> => {
    ...
    if (sess.presentResultCalledThisTurn && sess.lastPresentResultSummary) {
      assistantText = sess.lastPresentResultSummary;
      deps.sink.stream('\n\n' + assistantText);
    }
  ...
  const graph = new StateGraph(AgentState)
    ...
    .addConditionalEdges(AGENT_NODES.visualPreview, routeAfterDiscovery, [
      AGENT_NODES.visualPreview,
      AGENT_NODES.smEntry,
      END,
    ])
