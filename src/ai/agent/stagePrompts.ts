...
export function buildWorkerHopMessage(hop: HopContext | null, focusId: string): string {
  ...
  if (hop) {
    ...
    body = JSON.stringify(hopForJson, null, 2);
  } else {
    body = JSON.stringify({ focus_node_id: focusId });
  }
  // Same escape idiom as toolAttempt's <runtime_tool_context>: DDL/comments inside the hop JSON
  // must not be able to close the delimiter or smuggle markup into the instruction stream.
