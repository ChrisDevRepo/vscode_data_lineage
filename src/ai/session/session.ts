...
export function sameExplorationProposal(
  ...
  return JSON.stringify(left) === JSON.stringify(rightWithoutRevision);
}
...
export class AiSession {
  ...
  /** Clears a proposal after cancel or successful atomic activation. */
  public clearPendingExploration(token: number): SessionWriteOutcome {
    const guard = this.guardTurnWrite(token, 'clearPendingExploration');
    if (guard.kind === 'accepted') this.pendingExploration = null;
    return guard;
  }
