...
/** Type for GetScopeBundle tool input. */
...
const HighlightGroupSchema = z.object({
  ...
  node_ids: z.array(z.string()).describe('Node IDs that share this graph role or status.'),
}).strict();
...
export const PresentResultModelSchema = z.object({
  ...
  prune_node_ids: z.array(z.string()).optional().describe('ONLY permitted during Completed Phase follow-ups. Strictly forbidden during the initial Synthesis Phase.'),
  add_node_ids: z.array(z.string()).optional().describe('ONLY permitted during Completed Phase follow-ups. Strictly forbidden during the initial Synthesis Phase.'),
  ...
  sections: z.array(z.object({
    ...
    node_ids: z.array(z.string()).optional().describe('A node ID can only appear in ONE section. Do not link a node to multiple sections.'),
  ...
  notes: z.array(z.object({
    node_id: z.string().describe('Node ID receiving this below-node caption.'),
...
export const PresentResultVisualPreviewModelSchema = PresentResultModelSchema.omit({
  name: true,
  summary: true,
  title: true,
  intro: true,
