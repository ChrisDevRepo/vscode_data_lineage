...
export const TOOL_DEFS = [
  ...
  },
  {
    ...
    modelDescription: 'Grep over view, procedure and function bodies: a regular expression in, every match out with its object, 1-based line number, matched line and surrounding context, a hit inside a SQL comment flagged `commented: true` and those lines restated once per object in `commented_spans`. Case-insensitive; `^` and `$` match per line. No matches is `total: 0`; an unusable pattern is `invalid_regex` with the repair.',
