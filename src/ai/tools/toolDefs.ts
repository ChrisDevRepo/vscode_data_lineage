...
export const TOOL_DEFS = [
  ...
  },
  {
    ...
    modelDescription: 'Grep view, procedure and function bodies. Each hit has object, line, matched line, context, and `commented: true` when the match sits in a SQL comment. `by_object` is the per-object hit count. No matches is `total: 0`; a bad pattern is `invalid_regex` with the repair. Use lineage_search_objects for names.',
