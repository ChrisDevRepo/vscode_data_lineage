...
export function buildSynthesisReminder(question: string): string {
  return (
    ...
    '- Preservation target: each standalone badged slot → a section at comparable length (aim ≥ 50% of the slot\'s analysis length). Consolidated variant-sibling sections (e.g. Alloc1a+1b+1c merged) may compress to ≥ 30% of combined slot length. Two preservation layers within that target: (1) direct answer to the user question (formulas, column meanings, transformation logic, routing predicates) — format, sequence, explain the reasoning; do not compress the semantic content. (2) analyst-noticed enrichment the user should see (⚠️ risks, silent-failure modes, edge cases, anti-patterns, schema gotchas like nvarchar(1) truncation) — lift the full set when mission-relevant. Paraphrasing "EVDirect = EVBudget × 25%" into "25% allocation" drops layer-1 evidence; keeping only the single most dramatic ⚠️ out of five drops four pieces of layer-2 context.\n' +
