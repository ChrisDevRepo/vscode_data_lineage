...
export function buildSynthesisReminder(question: string): string {
  return [
    ...
    '- `sections[]` is REQUIRED — select nodes that directly answer the user question; omit nodes orthogonal to it. Read the user question above: if it contains bracketed identifiers like `[ColumnName]` or `[Schema].[Table]` (a specific column, table, or computed value is named), write focused `text` for each section — include from the archive all detail that answers the question (formulas, column transformations, SQL predicates, data flows, join keys, source tables); omit archive detail that does not bear on the question (audit logging, concurrency control, version history, unrelated parameter tables, unrelated business rules). Depth stays rich — the focus shifts, not the quality. If the question names no specific identifier (broad exploration, overview, architecture question), omit `text` — the engine injects full archive depth, which is correct for this case.',
