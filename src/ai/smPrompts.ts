...
export function buildSynthesisPrompt(): string {
  return [
    ...
    '',
    '### TASK — IN THIS EXACT ORDER, SAME TURN',
    '',
    '**STEP 1 — WRITE THE FULL CHAT REPLY FIRST (prose, before any tool call).**',
    'The chat reply is the USER\'S PRIMARY OUTPUT — they read it in the chat window and decide whether to act. A short "Done, see the graph" wrap-up is NOT acceptable. Write the full analytical report directly as your response, structured as:',
    '  - Open with a 2-3 sentence **executive answer** to the root question.',
    '  - Then one `## N. <node name>` section PER archive slot — if the archive has 27 slots, the chat reply must have 27 sections. Each section:',
    '    - **Business Purpose**: 1-2 sentences on the domain role (not "stores data" — name the actual business function).',
    '    - **Transforms**: SQL evidence + LaTeX formulas for every computation (`$expr = ...$` inline). Multi-step logic as ordered 1./2./3.',
    '    - **Column I/O**: markdown table `| Input | Source Column | Transform | Output |` whenever renames or computations occur. Name every column explicitly.',
    '    - **Relationships**: upstream / downstream in this flow.',
    '    - **Risks/Notes**: nullability, precision, edge cases (⚠️ prefix for data-quality risks).',
    '  - Variant siblings (e.g. `spCadenceRule_Alloc1a/1b/1c/1d`) each get their OWN section. Delta-mode wording is allowed ("Same skeleton as 1a; deltas: <named columns, named expressions, named conditions>") but each variant must have its own `##` heading.',
    '  - Close with 2-3 sentence **wrap-up** highlighting cross-cutting insights (e.g. allocation waterfall order, EV case coverage).',
    '',
    '**LENGTH FLOOR FOR CHAT PROSE**: ≥ the combined length of the archive\'s `analysis` fields. If the archive totals ~56K chars, the chat reply text must be ≥ 56K chars. A chat reply under 500 chars or with fewer sections than archive slots is a protocol violation — it tells the user the engine failed, even though you did the work. Under no circumstances reply with only "Done" / "See the graph" / "View created".',
    '',
    '**STEP 2 — AFTER the prose, call `lineage_enrich_view`** with `sections[]` mirroring the chat prose: one section per archive slot, `label = slot.badge_label`, `node_ids = [slot.nodeId]`, `text = the same per-slot content you wrote in the chat prose`. Plus `notes[]` one per node with `text = slot.note_caption`. `sections[]` is MANDATORY. Notes-only payloads produce a blank-looking graph and break the pipeline.',
    '',
    'Write the prose and then the tool call in the same response turn. Both deliver the same per-slot depth — prose is for reading in chat, the view is for graph navigation. Shipping one with full content and the other empty is a bug.',
  ].join('\n');
}
