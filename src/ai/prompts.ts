...
export function buildActivePhasePrompt(isInline: boolean): string {
  ...
  return [
    ...
    '3. MATHEMATICS: Express every formula in LaTeX — `$expr$` inline, `$$expr$$` block — transforms, allocations, thresholds, proportions, CASE expressions. Math captured this way reaches the final document; math expressed as prose does not.',
...
export function buildSynthesisPrompt(): string {
  return [
    ...
    '',
    'Your job: call `lineage_present_result` with `summary`, `title`, `intro`, **`sections[]`**, and optional `closing` / `notes` / `highlight_groups`. The engine assembles the rendered document (description, section numbering, badge chips, object link headers) deterministically from these parts. The captured detail bodies belong in `sections[]` — `intro` is a 2–4 sentence headline, not the whole report.',
    '',
    '## sections[] — REQUIRED',
    'Group along TWO orthogonal axes:',
    '1. **Angle** — every captured `slot.sections[i]` keeps its `angle` (`"business"` or `"technical"`). A business slot and a technical slot are NEVER merged into one final section, even when they describe the same node. Under `classification = both` you get two parallel section streams; under `business`/`technical` you get one stream.',
    ...
    '- `text`: LIFT VERBATIM the captured `slot.sections[i].text` bodies for the grouped nodes. When ≥2 nodes are grouped, hoist the shared formula/skeleton ONCE at the top and put differing fields into a comparison table (rows = variants, columns = differing fields). Preserve every LaTeX formula, every column-rename table, every ⚠️ risk callout — verbatim, no rewriting.',
    '',
    'Pass / Prune slots: mention in one line inside a `### Passthrough / Pruned` subsection — do not expand their text.',
    '',
    '## Other parts',
