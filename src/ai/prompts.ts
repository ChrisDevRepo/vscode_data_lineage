...
export function buildSynthesisPrompt(): string {
  return [
    ...
    '- Group same-shape sibling slots into one entry when the variants share a formula',
    '  or SQL skeleton: list every variant id in `node_ids[]`, hoist the shared',
    '  expression once above a comparison table (rows = variants, columns = differing',
    '  fields), preserve every ⚠️ per row. Distinct logic always gets its own entry.',
    ...
    '',
    '<example name="variant-table-grouping">',
    'Three procedures (`spLoadEU`, `spLoadUS`, `spLoadAPAC`) share the same upsert skeleton',
    'with regional deltas. Captured slots for each carry the same MERGE shape, differing',
    'only on source table, region filter, and currency conversion.',
    '',
    'Combined into one synthesis section:',
    '',
    '```',
    'label: "Regional Sales Upsert"',
    'node_ids: ["[etl].[spLoadEU]", "[etl].[spLoadUS]", "[etl].[spLoadAPAC]"]',
    'text:',
    '  ## Purpose',
    '  Region-partitioned upsert into `FactSalesReport` from staged regional extracts.',
    '  Shared formula:  $TotalRevenue = Qty \\times UnitPrice \\times FxRate$',
    '',
    '  | Region | Source | Region filter | FxRate column |',
    '  | --- | --- | --- | --- |',
    '  | EU | `[stg].[OrdersEU]` | `Country IN (\'DE\',\'FR\',...)` | `EurToBaseFx` |',
    '  | US | `[stg].[OrdersUS]` | `Country = \'US\'` | `1.0` (constant) |',
    '  | APAC | `[stg].[OrdersAPAC]` | `Country IN (\'JP\',\'AU\',...)` | `LocalToBaseFx` |',
    '',
    '  ⚠️ APAC FxRate lookup misses on new `Country` codes — silent NULL collapses revenue to 0.',
    '  ⚠️ US row hardcodes FxRate = 1.0; remove if base currency changes.',
    '```',
    '</example>',
  ].join('\n');
}
