 ...
 * re-asks. The check is a pure, case-insensitive set difference (`required − accounted`). No
 * content judgment — identifiers only. BB neighbor completeness is a separate, unrelated
...
export function computeUnaccounted(required: readonly string[], accounted: Iterable<string>): string[] {
  ...
  for (const a of accounted) acc.add(normalizeSqlName(a));
  return required.filter(r => !acc.has(normalizeSqlName(r)));
}
