    ...
    <>
      <div className="ln-toolbar flex items-center gap-2 px-4 py-2.5">
        ...
        <SearchWithAutocomplete
          ...
          types={types}
        />
        <Button onClick={onToggleDetailSearch} variant={isDetailSearchOpen ? 'primary' : 'icon'} title="Detail Search (full-text search in SQL bodies)">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
        ...
        {/* View Controls */}
        <Button onClick={onOpenDdlViewer} variant="icon" title={hasHighlightedNode ? 'Open SQL Viewer for selected node' : 'Open SQL Viewer'}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
            <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
          </svg>
        </Button>
        <Button onClick={onToggleIsolated} variant={hideIsolated ? 'default' : 'ghost'} title={analysisType === 'orphans' ? 'Disabled during Orphan analysis' : 'Hide Isolated Nodes'} disabled={analysisType === 'orphans'}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
        ...
        <div className="relative" ref={analysisDropdownRef}>
          <Button onClick={() => setIsAnalysisDropdownOpen(prev => !prev)} variant={isAnalysisActive ? 'primary' : 'ghost'} title="Graph Analysis">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
          ...
          {isAnalysisDropdownOpen && (
            <div className="absolute top-full left-0 mt-1 w-48 rounded-md shadow-lg z-50 ln-dropdown" style={{ boxShadow: 'var(--ln-dropdown-shadow)' }}>
              <div className="py-1">
                <button className="w-full text-left px-3 py-1.5 text-xs ln-list-item flex items-center gap-2" onClick={() => { setIsAnalysisDropdownOpen(false); onOpenAnalysis?.('islands'); }}>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-3.5 h-3.5"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" /></svg>
                  Islands
                </button>
                <button className="w-full text-left px-3 py-1.5 text-xs ln-list-item flex items-center gap-2" onClick={() => { setIsAnalysisDropdownOpen(false); onOpenAnalysis?.('hubs'); }}>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-3.5 h-3.5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-4.773-4.227-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z" /></svg>
                  Hubs
                </button>
                <button className="w-full text-left px-3 py-1.5 text-xs ln-list-item flex items-center gap-2" onClick={() => { setIsAnalysisDropdownOpen(false); onOpenAnalysis?.('orphans'); }}>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-3.5 h-3.5"><path strokeLinecap="round" strokeLinejoin="round" d="M18.364 18.364A9 9 0 0 0 5.636 5.636m12.728 12.728A9 9 0 0 1 5.636 5.636m12.728 12.728L5.636 5.636" /></svg>
                  Orphan Nodes
                </button>
                <button className="w-full text-left px-3 py-1.5 text-xs ln-list-item flex items-center gap-2" onClick={() => { setIsAnalysisDropdownOpen(false); onOpenAnalysis?.('longest-path'); }}>
                ...
                <button className="w-full text-left px-3 py-1.5 text-xs ln-list-item flex items-center gap-2" onClick={() => { setIsAnalysisDropdownOpen(false); onOpenAnalysis?.('cycles'); }}>
