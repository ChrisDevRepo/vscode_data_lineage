  ...
  return (
    <>
      <div className="ln-toolbar flex items-center gap-2 px-4 py-2.5">
        {/* Navigation */}
        <Button onClick={onBack} variant="icon" title="Back to Project Selection">
        ...
        <SearchWithAutocomplete
          ...
          types={types}
        />
        <Button onClick={onToggleDetailSearch} variant="icon" className={isDetailSearchOpen ? 'ln-btn-icon-active' : ''} title="Detail Search (full-text search in SQL bodies)">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
        ...
        <Button onClick={onToggleIsolated} variant="icon" className={hideIsolated ? 'ln-btn-icon-active' : ''} title={analysisType === 'orphans' ? 'Disabled during Orphan analysis' : 'Hide Isolated Nodes'} disabled={analysisType === 'orphans'}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
        ...
        <div className="relative" ref={analysisDropdownRef}>
          <Button onClick={() => setIsAnalysisDropdownOpen(prev => !prev)} variant="icon" className={isAnalysisActive ? 'ln-btn-icon-active' : ''} title="Graph Analysis">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
              <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 3v11.25A2.25 2.25 0 0 0 6 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0 1 18 16.5h-2.25m-7.5 0h7.5m-7.5 0-1 3m8.5-3 1 3m0 0 .5 1.5m-.5-1.5h-9.5m0 0-.5 1.5m.75-9 3-3 2.148 2.148A12.061 12.061 0 0 1 16.5 7.605" />
            </svg>
          </Button>
          {isAnalysisDropdownOpen && (
            <div className="absolute top-full left-0 mt-1 w-52 rounded-md shadow-lg z-50 ln-dropdown" style={{ boxShadow: 'var(--ln-dropdown-shadow)' }}>
              <div className="py-1">
                <button className="w-full text-left px-3 py-1.5 text-sm ln-list-item flex items-center gap-2" onClick={() => { setIsAnalysisDropdownOpen(false); onOpenAnalysis?.('islands'); }}>
                ...
                <button className="w-full text-left px-3 py-1.5 text-sm ln-list-item flex items-center gap-2" onClick={() => { setIsAnalysisDropdownOpen(false); onOpenAnalysis?.('hubs'); }}>
                ...
                <button className="w-full text-left px-3 py-1.5 text-sm ln-list-item flex items-center gap-2" onClick={() => { setIsAnalysisDropdownOpen(false); onOpenAnalysis?.('orphans'); }}>
                ...
                <button className="w-full text-left px-3 py-1.5 text-sm ln-list-item flex items-center gap-2" onClick={() => { setIsAnalysisDropdownOpen(false); onOpenAnalysis?.('longest-path'); }}>
        ...
        {/* Reset & Refresh */}
        <Button onClick={onRefresh} variant="icon" title="Clear All Filters">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
        ...
        {onRebuild && (
          <Button onClick={onRebuild} variant="icon" title="Refresh (re-read settings & rebuild layout)">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
        ...
        {/* Export & Help */}
        <Button onClick={onExportDrawio} variant="icon" title="Export as Draw.io">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
