...
function registerChatParticipant(context: vscode.ExtensionContext, getSession: () => AiSession, outputChannel: vscode.LogOutputChannel) {
  const participant = vscode.chat.createChatParticipant(
    ...
    async (request, chatContext, stream, token): Promise<vscode.ChatResult> => {
        ...
        if (turn instanceof vscode.ChatRequestTurn) {
          ...
          if (meta?.toolCallRounds?.length) {
            for (const round of meta.toolCallRounds) {
              ...
              historyMessages.push(new vscode.LanguageModelChatMessage(vscode.LanguageModelChatMessageRole.Assistant, assistantParts));
              ...
              for (const tc of round.toolCalls) {
                ...
                if (r) {
                  ...
                  const compact = compactNoiseResult(tc.name, contentStr) || compactStaleHopResult(tc.name, contentStr, sess.stateMachine?.status === 'complete', false);
                  resultParts.push(new vscode.LanguageModelToolResultPart(tc.callId, [new vscode.LanguageModelTextPart(compact || contentStr)]));
                }
              }
      ...
      const serializeMessages = (msgs: vscode.LanguageModelChatMessage[]): string => msgs.map(m => typeof m.content === 'string' ? m.content : JSON.stringify(m.content)).join('\n');
      ...
      const runWithTools = async () => {
        ...
        while (roundCount < MAX_ROUNDS) {
          ...
          for (const call of toolCalls) {
            ...
            if (cached) {
              resultParts.push(new vscode.LanguageModelToolResultPart(call.callId, [new vscode.LanguageModelTextPart(JSON.stringify({ _dedup: true }))]));
              continue;
            }
            ...
            stream.progress(`Invoking ${call.name.replace('lineage_', '')}...`);
            totalToolCallsMade++;
            try {
          ...
          for (const call of toolCalls) {
            const res = accumulatedToolResults[call.callId];
            if (res) {
      ...
      try {
        ...
        if (activePanel && (hasBfs || smComplete)) {
