...
async function loadAiOutputTemplates(
  ...
  const logger = Logger.create(outputChannel, 'Config');
  const REQUIRED_KEYS: (keyof AiOutputTemplates)[] = ['summary', 'title', 'intro', 'description', 'sections', 'closing', 'highlights', 'notes', 'business_capture', 'technical_capture', 'business_subsection', 'technical_subsection', 'structural_summary'];
