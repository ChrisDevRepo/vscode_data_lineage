  ...
  try {
    if (aiEnabled && missingAiApis.length > 0) {
      notifyWarning(
        ...
        `Data Lineage: the AI assistant is unavailable — this editor does not provide ${missingAiApis.join(' or ')}. `
        + 'Lineage visualisation, parsing and the graph are unaffected.',
        { missing: missingAiApis.join(', ') },
      );
    } else if (aiEnabled) {
...
async function loadAiOutputTemplates(
  ...
  logger.debug(`Reading AI templates custom: ${customPath}`);
  ...
  try {
    ...
    if (customVersion !== AI_TEMPLATE_SCHEMA_VERSION) {
      logger.warn(
        `Custom AI output templates at "${customPath}" have schemaVersion ${String(customVersion ?? 'missing')} but this ` +
        `release expects ${AI_TEMPLATE_SCHEMA_VERSION} — the template structure has changed. Review and update your YAML ` +
        `(re-scaffold via "Create AI Output Templates" and re-apply your edits); using built-in defaults until then.`,
      );
      return builtIn;
    }
...
async function loadParseRules(
  ...
  for (const err of result.errors) logger.debug(`Skipped parse rule: ${err}`);
