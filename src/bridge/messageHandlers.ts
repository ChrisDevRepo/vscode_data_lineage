...
import {
  ...
  executeSimpleQuery, getServerInfo, withQueryTimeout, isPhase2Query, type DmvQuery,
 ...
 * labels set, and `dataLineageViz.modelLoaded` raised so the model-gated menu items appear.
 *
...
async function runDbPhase2Host(host: BridgeHost, connectionUri: string, schemas: string[], outputChannel: vscode.LogOutputChannel, getSession: () => AiSession, currentDatabase?: string, sourceName?: string, onModelBuilt?: (model: DatabaseModel) => void) {
  ...
  const timeoutMs = (host.getConfiguration().get<number>('dmvQueryTimeout') ?? 120) * 1000;
  // Platform detection is step 1 of the phase; the sweep's own 1..N steps shift up by one so
  // the counter stays monotonic instead of restarting when the sweep begins.
  const totalSteps = queries.filter(isPhase2Query).length + 1;
  ...
  const resultMap = await executeDmvQueriesFiltered(connectionUri, queries, schemas, outputChannel, (step, total, label) => {
    host.postMessage({ type: 'db-progress', step: step + 1, total: total + 1, label });
  }, timeoutMs);
