...
| O2 | Playground proof owed for follow-up wording `a4e7d559e`, `d73db5898` | owed | think first, then /prompt-change playground on an existing recording: a few reasoned options, 1–5 calls (row no-run-for-a-screen) |
| O4 | `follow-up-explore-next-autosupplement` — headless only (row no-pm-retest) | headless run stopped 2026-09-22 (brute force, row no-run-for-a-screen); partial recording `test-results/e2e/o4-explore-autosupplement/2026-09-22T05-45-25-391Z-azure-foundry` | fold into O2: reason out the expected follow-up behaviour, then screen it with the /prompt-change playground at a follow-up hop of an existing recording, a few options, 1–5 calls |
