...
| O2 | Playground proof owed for follow-up wording `a4e7d559e`, `d73db5898` | owed | `/prompt-change` playground only, 1–5 calls total, on an existing recording (row no-run-for-a-screen); no headless run to create one |
| O4 | `follow-up-explore-next-autosupplement` — headless only (row no-pm-retest) | STOPPED 2026-09-22 by PM (spend): headless run cut at 24 calls / 278k tokens, base turn only (`test-results/e2e/o4-explore-autosupplement/2026-09-22T05-45-25-391Z-azure-foundry`) | `--followup "Explore related objects"` then a pick: turn 2 lists + asks, turn 3 supplements |
