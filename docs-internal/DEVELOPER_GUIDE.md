...
- **Architecture**: The legacy Claude-based evaluation loop has been retired. AI correctness is now validated using the `vscode-test` framework calling real Copilot Chat (GPT-4o) tools.
- **Tiers**:
    - **Quick Suite** (`npm run test:ai:quick`): Shallow traces (10+ hops) and tool baselines. Run before every merge.
    - **Detail Suite** (`npm run test:ai:detail`): Deep explorations (15-30+ hops) and role-based scenarios (Performance, Documentation).
- **Quality Gate**: Prompt changes MUST be accompanied by a **Baseline Report** generated via `npm run test:baseline`. This report diffs the `enrich_view` payloads to ensure the AI's narrative quality, math accuracy, and section ordering have not degraded.
