...
### Prerequisites
- [Node.js](https://nodejs.org/) at the version declared by `.nvmrc` and
  `package.json` (`nvm use`).
...
| Tier | Command | Scope |
| :--- | :--- | :--- |
| **Full local gate** | `npm run gate` | Type-checking, unit tests, production builds, and package checks. |
| **Unit suite** | `npm test` | Every `tests/unit/**/*.test.ts` file. |
| **Core** | `npm run test:core` | All parser tests followed by the BFS/state-machine tests. |
| **Parsing** | `npm run test:parser` | DACPAC, DMV, T-SQL, and targeted SQL fixtures. |
| **BFS** | `npm run test:bfs` | Graph construction, traversal, `NavigationEngine`, state-machine contracts, result closure, and trace safety. |
| **Optional AI backend** | `npm run test:ai-backend` | Scripted selected-model/runtime check in a VS Code host; not part of the gate. |

Use the narrowest maintained command that matches the change:
- Parser or parse-rule work: `npm run test:parser`
- Graph construction, traversal, or `NavigationEngine`: `npm run test:bfs`
