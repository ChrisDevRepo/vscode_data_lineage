...
describe('migrateProjectStore', () => {
  ...
  it('drops projects containing unknown connection fields', () => {
    ...
    expect(s.projects, 'unknown credential field rejected').toEqual([]);
  });
...
describe('migrateProjectStore drop reporting', () => {
  it('reports the dropped count and offending field paths', () => {
    ...
    expect(s.projects.length, 'valid project retained').toBe(1);
    expect(reports.length, 'reported once for the whole store').toBe(1);
    expect(reports[0].dropped, 'both invalid records counted').toBe(2);
    expect(reports[0].issuePaths, 'missing id reported by path').toContain('id');
    expect(
      reports[0].issuePaths.some((p) => p.includes('password')),
      'unknown credential field reported by path',
    ).toBe(true);
