...
describe('parseTrace / serializeRun round trip', () => {
  ...
  it('round-trips an empty file to an empty run and back to an empty string', () => {
    ...
    expect(run).toMatchObject({
      ...
      traceOpen: [], turns: [], generations: [], tools: [], gates: [], phases: [], wire: [], raw: [], malformed: [],
    });
...
describe('unknown and malformed lines', () => {
  ...
  it('isolates the malformed line without losing any well-formed neighbor', () => {
    ...
    expect(run.traceOpen.length + run.turns.length + run.generations.length + run.tools.length
...
describe('typed accessor bucket counts', () => {
  ...
  it('types the producer stamp: the trace-open origin is readable without touching raw[]', () => {
    expect(run.traceOpen[0]).toMatchObject({ type: 'trace-open', origin: 'headless-harness', verbose: false });
  });
  ...
  it('assigns each line a distinct, source-ordered lineIndex', () => {
    ...
    expect(Math.max(...indices, ...run.malformed.map((entry) => entry.line))).toBe(28);
  });
});
