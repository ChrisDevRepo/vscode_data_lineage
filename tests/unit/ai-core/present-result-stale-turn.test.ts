...
describe('executePresentResult — turn-lease enforcement', () => {
  ...
  it('returns stale_turn from the commit guard when the turn is superseded mid-handler', async () => {
    ...
    // Documented current behaviour: the unguarded resultGraph writes at the end of the handler run
    // BEFORE the guarded commit, so they survive a rejected commit. Pinned here so a change to the
    // handler's write ordering is a visible test change, not a silent one.
    expect(resultGraph.summary).toBe('How Orders is populated.');
    expect(resultGraph.notes).toEqual([{ nodeId: ORIGIN_NODE, summary: 'Orders is the sole source table.' }]);
  });
