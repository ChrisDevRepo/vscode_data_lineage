...
/**
 * A provider that answers with a complete, schema-valid tool payload fenced as prose instead of a
 * native tool-call chunk must be promoted rather than charged a synthetic `missing_required_tool_call`
 * rejection. Fixtures below are verbatim `wire-response.text` values covering the encoding shapes
 * seen on the wire: fenced JSON, unfenced JSON, and a Hermes/XML tool-call envelope.
 */

// spRefreshSegments, no column_flow (BB shape).
...
// vwRawOrders, carries column_flow (CT shape).
...
// it and it must stay on the unpromoted rejection path.
...
// A complete, schema-valid call carried in the Hermes/XML <tool_call><function=...><parameter=...>
// envelope instead of fenced or unfenced JSON.
...
describe('VscodeModelPort prose tool-call promotion (C2)', () => {
  it('promotes a fenced submit_findings payload with no column_flow (spRefreshSegments)', async () => {
  ...
  it('promotes a fenced submit_findings payload carrying column_flow (vwRawOrders)', async () => {
  ...
  it('promotes an unfenced submit_findings payload that is the whole message body (vwExternalOrders)', async () => {
  ...
  it('does not promote an unfenced payload that fails the tool schema (no focus_node_id)', async () => {
  ...
  // A 55-character `badge_label` is a content cap: advertised in the JSON schema, enforced by
  // `NavigationEngine.submitFindings`, which rejects the field alone against a held draft. The port
  // promotes the hop rather than discarding a complete analysis at the wire, carrying the over-long
  // label through verbatim for the engine to judge.
  it('promotes a fenced payload whose only defect is a content cap the engine owns', async () => {
    const port = portOver([N15_CONTENT_CAP_PAYLOAD]);
    const result = await port.generateToolTurn({
      messages: [new HumanMessage('analyze')],
      tools: [SUBMIT_FINDINGS_TOOL],
      toolChoice: 'required',
    ...
    expect(result.toolCalls).toHaveLength(1);
    expect(result.toolCalls[0]).toMatchObject({ valid: true, toolName: 'lineage_submit_findings' });
    const promoted = (result.toolCalls[0] as { input: { badge_label: string; focus_node_id: string } }).input;
    expect(promoted.focus_node_id).toBe('[ai].[sploadsalesstaging]');
    expect(promoted.badge_label, 'the over-long label reaches the engine unmodified').toHaveLength(55);
    expect(result.text).toBe('');
  });

  // The fenced half of the promotion guard: a payload whose defect is STRUCTURAL — a missing
  // required field, not a content size — is never promoted.
  it('does not promote a fenced payload that fails the tool schema (no focus_node_id)', async () => {
    const fenced = `\n\n\`\`\`json\n${T7_UNFENCED_SCHEMA_INVALID_PAYLOAD}\n\`\`\``;
    const port = portOver([fenced]);
    const result = await port.generateToolTurn({
      messages: [new HumanMessage('analyze')],
      tools: [SUBMIT_FINDINGS_TOOL],
      toolChoice: 'required',
  ...
  it('promotes a Hermes/XML <function=…> envelope (spImportOrders)', async () => {
