 ...
 * that rejection against — archived with the UAT capture.
 */
...
// otherwise well-formed payload whose only defect is a content cap (`badge_label` at 55 chars
// against a 50-char max) — a cap the engine owns, so the hop is promoted and judged there.
...
// azure-foundry, recorded T8 lm-trace generation 13 (phase=active) — the
...
// azure-foundry, recorded T7 lm-trace generation 14 (phase=active) —
...
// local-mlx (Hermes grammar), recorded T5 lm-trace line 46 (phase=active) -- a complete, schema-valid
...
describe('VscodeModelPort prose tool-call promotion (C2)', () => {
  ...
  // The archived n15 payload's only defect was a 55-character `badge_label`. That cap is a content
  // cap: advertised in the JSON schema, enforced by `NavigationEngine.submitFindings`, which rejects
  // the field alone against a held draft (pinned in tests/unit/sm/submit-findings-handler.test.ts).
  // The port therefore promotes the hop rather than discarding a complete analysis at the wire, and
  // carries the over-long label through verbatim for the engine to judge.
  it('promotes the archived n15 payload whose only defect is a content cap the engine owns', async () => {
  ...
  // The fenced half of the promotion guard, which the n15 case above used to carry: a payload whose
  // defect is STRUCTURAL — a missing required field, not a content size — is never promoted.
