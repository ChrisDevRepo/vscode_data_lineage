...
describe('Present Result Closure', () => {
  it('runs all present-result closure assertions', () => {
...

    {
      ...
      assert(
        result.success === false && result.errors.some(e => e.includes('invalid KaTeX math')),
        'rejects block math that the webview KaTeX renderer cannot parse',
      );
      assert(
        result.success === false && isRepairablePresentResultFailure(result),
        'invalid section math authorizes a held-draft sections repair',
      );
    }

    {
      ...
      const assembled = orderAndAssemble(sections);
      const result = validatePresentResult({
        name: 'bad-strict-math',
        summary: 'bad strict math',
        sections,
        highlight_groups: [{ label: 'Flow', color: 'source', node_ids: ['a'] }],
      }, ['a'], assembled.badges, assembled.description);
      assert(
        result.success === false && result.errors.some(e => e.includes('invalid KaTeX math')),
        'rejects strict KaTeX failures such as unescaped percent signs',
      );
    }

    {
      ...
      const assembled = orderAndAssemble(sections);
      const result = validatePresentResult({
        name: 'bad-inline-code',
        summary: 'bad inline code',
        sections,
        highlight_groups: [{ label: 'Flow', color: 'source', node_ids: ['a'] }],
      }, ['a'], assembled.badges, assembled.description);
      assert(
        result.success === false && result.errors.some(e => e.includes('unclosed inline code span')),
        'rejects an unmatched inline-code delimiter',
      );
    }
    ...
    // must be individually repairable, matching the AND-semantics the old string-match .every() had.
    {
      const sections = [{ label: 'Source', text: 'One.' }];
      const assembled = orderAndAssemble(sections);
      const result = validatePresentResult({
