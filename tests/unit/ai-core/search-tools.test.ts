...
describe('search tools — grep contract', () => {
  ...
  it('marks a hit inside a comment and leaves a live hit\'s row byte-identical', () => {
    ...
    // The wire shape: a live row keeps its fields and their key order; the flag is appended only
    // where it is true. Its `context` marks the dead line it reaches, which is the whole point —
    // the flag answers for the MATCHED line, and a snippet is several lines wide.
