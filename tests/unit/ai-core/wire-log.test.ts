...
/** Every temporary trace root this file created, so none survives the run. */
const traceRoots: string[] = [];

/**
 * Creates a temporary directory for one `AiTraceWriter` and registers it for removal.
 *
 * @returns Absolute path of the new directory.
 *
 * @remarks
 * `AiTraceWriter.enable` writes a real NDJSON file, so these cases need a real directory rather
 * than a mocked filesystem. Registering here rather than deleting per case keeps the removal off
 * the assertion path: a failing case still leaves its trace readable until the file finishes.
 */
async function traceRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'lineage-trace-'));
  traceRoots.push(root);
  return root;
}

afterAll(async () => {
  await Promise.all(traceRoots.map((root) => rm(root, { recursive: true, force: true })));
});
...
describe('model wire log', () => {
  ...
  it('writes lifecycle and wire records to one session trace after the command enables it', async () => {
    ...
    const [opened, lifecycle, request, response, generation] =
      lines.map((line) => JSON.parse(line) as Record<string, unknown>);

    // Line 1 always names the producer, so a file is never ambiguous about where it came from.
    expect(opened).toMatchObject({ type: 'trace-open', origin: 'extension-host' });
...
describe('safeTraceStringify', () => {
  ...
  it('keeps Date, Map, and Set content instead of flattening them to {}', () => {
    // These carry no own enumerable keys, so a generic key walk records `{}` — for a Date that is
    // strictly worse than plain `JSON.stringify`, which emits the ISO string through `toJSON`.
    const parsed = JSON.parse(safeTraceStringify({
      createdAt: new Date('2026-02-01T10:30:00.000Z'),
      invalid: new Date(Number.NaN),
      byName: new Map([['dbo.Orders', 3]]),
      seen: new Set(['a', 'b']),
    })) as Record<string, unknown>;

    expect(parsed.createdAt).toBe('2026-02-01T10:30:00.000Z');
    expect(parsed.invalid).toBe('[Invalid Date]');
    expect(parsed.byName).toEqual({ '[Map]': [['dbo.Orders', 3]] });
    expect(parsed.seen).toEqual({ '[Set]': ['a', 'b'] });
  });
