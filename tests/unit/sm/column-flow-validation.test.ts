...
describe("Column Flow Validation", () => {
  ...
  // The donor literal here used a stale pre-DatabaseModel shape (Set schemas, an
  // objectMap field that was never part of DatabaseModel). validateColumnFlow() only
  // ever reads model.neighborIndex (and only for 'procedure'-typed upstream
  // contributors, none of which appear in this test), so an empty makeModel() is a
  // like-for-like replacement, not a behavior change.
