 ...
 *
 * NOTE (conversion fix): the donor script called `setDiscoveryNodeCap`/`setDiscoveryTokenBudget`
 * (process-wide mutable module state in src/ai/tools/tools.ts) between scenario blocks, relying
 * on top-to-bottom execution to make each cap value "current" when the next block ran. Native
 * vitest registers every it() during synchronous collection and only runs the callbacks
 * afterward, so a cap-setting call left outside an it() would fire during collection — before any
 * test runs — and every it() would see only the LAST cap value set, not the one the donor script
 * intended at that point. Each cap-setting call below is moved inside the it() that immediately
 * followed it in the donor source (same relative order, vitest runs it()s in a file sequentially
 * by default), so the sequential mutate-then-read narrative is reproduced exactly. Likewise
 * `await loadDemoModel()` was a bare top-level await outside any async function (a hard syntax
 * error); it now runs in `beforeAll`, with `model`/`graph`/`origin` populated before any it() runs.
 */
