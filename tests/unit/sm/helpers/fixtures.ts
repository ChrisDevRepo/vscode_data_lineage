 ...
 *
 * These synthetic NavigationEngine test graphs were restored verbatim from
 * donor/testing13, where they never type-checked either: `LineageNode.fullName`,
 * `DatabaseModel.schemas: SchemaInfo[]`, and the `'body' | 'exec'` LineageEdge.type
 * union were all tightened after the donor fixtures were written (fixture-shape
 * drift, not a behavioral gap — `npm test` already passes 442/442 against the
 * untyped literals).
 *
 ...
 *     including the donor's literal `'SELECT'`, to `'read'` — the same fallback it
 ...
 * is tagged `'body'`; see the module doc for why this is behaviorally inert versus the
 * donor's non-member `'SELECT'` literal.
 */
