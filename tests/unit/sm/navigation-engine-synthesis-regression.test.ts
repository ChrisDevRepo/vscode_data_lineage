...
describe("Navigation Engine Synthesis Regression", () => {
  ...
  // Vitest runs describe-body setup once during collection and defers every it() callback to a
  // later run phase, so a shared reassigned `let` would leave all three it()s observing only its
  // FINAL value — silently making "Hop 1"/"Hop 2" check hop 3's context instead of their own. Each
  // checkpoint is snapshotted into its own const immediately after the engine call that produces
  // it, preserving the exact sequential engine mutations and exactly which snapshot each assertion
  // checks.
