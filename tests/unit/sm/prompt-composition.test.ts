...
describe('prompt composition', () => {
  ...
  // Two falsehoods the follow-up protocol used to assert: that the archive and the rendered
  // sections ride into context (history replay carries user turns + assistant markdown only), and
  // that sections[] is "updated" (lineage_present_result replaces the list wholesale —
  // presentResult.ts assigns resultGraph.sections from the payload, so omission is deletion).
