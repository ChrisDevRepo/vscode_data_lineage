 ...
 *
 * The DMV lane no longer supplies `allObjects` (its `all-objects` query was unreachable and was
 * removed), so the DACPAC lane is now the only producer of a catalog wider than the rendered
 * node set. The equivalent DMV-side tests were deleted with that query; without this, the branch
 * ships untested. Asserts what those tests asserted: an object outside the selected schemas is
 * still resolvable for display and still reachable through `neighborIndex`, without being
 * rendered as a node.
 */
async function testCrossSchemaCatalogUnderFilter() {
  ...
  const buf = readFileSync(testPath('AdventureWorks2025_AI.dacpac'));
  const { elements } = await extractSchemaPreview(buf.buffer as ArrayBuffer);
