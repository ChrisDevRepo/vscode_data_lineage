...
describe('parseAiOutputTemplatesYaml (assets/aiOutputTemplates.yaml)', () => {
  ...
  it('reads schemaVersion === 1', () => {
    expect(parseAiOutputTemplatesYaml(text).schemaVersion).toBe(1);
  });
...
describe('AiOutputTemplatesConfigSchema negative/positive cases', () => {
  it('rejects a scalar value under a template key', () => {
    // The bug this schema fixes: a top-level scalar under a non-schemaVersion key must
    // still reject (every key but schemaVersion must be a template object).
  ...
  it('accepts a bare top-level schemaVersion scalar and round-trips it', () => {
    // The bug itself: a bare top-level schemaVersion scalar must parse cleanly (it used to throw).
