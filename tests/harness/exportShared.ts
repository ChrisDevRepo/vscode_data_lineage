/**
 * Helpers shared by the two REST trace exporters (`langfuseExport.ts`, `langsmithExport.ts`).
 *
 * @remarks
 * Single home for the verbose-content gating rule and error redaction so the two exporters
 * cannot silently diverge in what they report. Plain data shaping only — no vendor SDK, no
 * network I/O, per the sanctioned plain-HTTP exporter policy in CLAUDE.md.
 */
