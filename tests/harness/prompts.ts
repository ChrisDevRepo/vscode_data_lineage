/**
 * The named prompt registry every live-provider run is launched from.
 *
 * @remarks
...
export interface PromptDefinition {
  ...
  /**
   * Whether this prompt's plan requires the run to reach the SM/graph pipeline's terminal
   * presentation tool.
   *
   * @remarks
   * `true` for a prompt that exists to exercise the state-machine loop (a business-blueprint or
   * column-trace run) — such a run is hollow, whatever route the model took to get there, unless it
   * ends in an accepted `lineage_present_result` call. `false` for a discovery prompt, whose plan is
   * satisfied by a non-empty final answer regardless of which tools it used along the way — a
   * discovery prompt is free to wander into the SM loop (T-prompts document this: a model picking a
   * different tool than the scripted case is a finding about the model, never a harness failure) and
   * is judged only on whether it answered.
   */
...
export interface ResolvedPrompt {
  ...
  /** Copied from {@link PromptDefinition.expectsGraphPhase}; absent for free text — no known plan. */
  readonly expectsGraphPhase?: boolean;
}
...
function definition(
  ...
  intent: string,
  text: string,
  expectsGraphPhase: boolean,
): PromptDefinition {
  return { id, family, intent, text, expectsGraphPhase };
}
...
export function resolvePrompt(value: string): ResolvedPrompt {
  ...
  if (registered) {
    return {
      ...
      source: 'registry',
      expectsGraphPhase: registered.expectsGraphPhase,
    };
  }
