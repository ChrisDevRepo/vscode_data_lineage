 ...
 *
 * - **Configuration answers are the caller's own defaults, VERBATIM.** `get(key, default)` returns
 *   `default`; `get(key)` returns `undefined`. A shim that invented plausible numbers would turn
 *   every budget-sensitive measurement into a measurement of the shim.
