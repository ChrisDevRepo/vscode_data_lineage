 ...
 * {@link AiTraceRecord} — the lifecycle union (`trace-open`, `turn-start`, `turn-terminal`,
 * `gate`, `phase`, `tool`) plus the wire union (`wire-request`, `wire-response`, `wire-error`,
 * `generation`, `provider-raw`). This module turns that byte stream back into typed buckets without ever
...
export const TRACE_MODEL_SCHEMA_VERSION = 2;
...
export type TraceOpenEntry = TraceEntry<Lifecycle<'trace-open'>>;
...
export interface ParsedRun {
  ...
  /** The producer stamp(s) — the writer emits exactly one as the file's first line; concatenated captures may carry more. */
  readonly traceOpen: readonly TraceOpenEntry[];
...
export function parseTrace(ndjson: string): ParsedRun {
  ...
  const traceOpen: TraceOpenEntry[] = [];
  ...
  lines.forEach((text, lineIndex) => {
    ...
    switch (entry.type) {
      case 'trace-open':
        traceOpen.push(entry as TraceOpenEntry);
        return;
      case 'turn-start':
      case 'turn-terminal':
  ...
  return { schemaVersion: TRACE_MODEL_SCHEMA_VERSION, traceOpen, turns, generations, tools, gates, phases, wire, raw, malformed };
}
...
export function serializeRun(run: ParsedRun): string {
  ...
  const typed: ReadonlyArray<TraceEntry<Record<string, unknown>>> = [
    ...run.traceOpen, ...run.turns, ...run.generations, ...run.tools, ...run.gates, ...run.phases,
    ...run.wire, ...run.raw,
  ];
