...
Press <kbd>?</kbd> in the graph webview for the maintained shortcut list.
Shortcuts are ignored while typing in editable controls, and the extension does
not register VS Code global keybindings.
...
- **Refresh View** clears active filters, collapses expanded schemas, and re-applies the initial Object View / Schema View decision.
...
- Profiling lifecycle events and a bounded SQL preview are logged to the Output channel (`View → Output → Data Lineage Viz`) at DEBUG level.
