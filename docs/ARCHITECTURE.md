...
| Diagnostic trace | [`src/ai/observability/`](../src/ai/observability/) | `wireLog` request/response/error/generation/provider-raw records and the `aiTraceWriter` session NDJSON sink. `wireLog.ts` is `vscode`-free so every model port emits the same record surface; the VS Code message projection lives in `vscodeWireLog.ts`. |
...
summary and renders every in-scope object for approval or cancellation.
The native chat gate exposes **Approve & Proceed**, which resumes the
gate with the proposed classes, and **Change scope / Cancel**, which opens a
native choice between **Change scope** and **Cancel**. Dismissing that choice
keeps the gate pending. **Cancel** clears the pending proposal without creating
an engine. **Change scope** opens a native input box; dismissing it or submitting
blank text keeps the reviewed proposal unchanged.

A nonblank change resumes the same interrupted LangGraph turn through the
...
revision. A failed or no-op patch leaves the previous revision pending. Buttons
belonging to a stale, replaced, or already-resolving gate are rejected with a
warning and change no state. Scope-expansion gates keep a plain **Cancel** button.
No `NavigationEngine` is published as active until approval succeeds.
...
extension does not add a transport retry, timeout, model fallback, or duplicate
retry UI: VS Code owns Stop/cancellation and the native whole-request Retry
action. Provider failures settle once through `ChatResult.errorDetails`; graph
loops remain limited to semantic repair with fresh model generations.
...
checked with `npm run test:e2e` in the Extension Development Host; see
