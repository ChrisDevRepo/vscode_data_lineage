...
- **Runtime query names** — the current import path requires
  `schema-preview`, `nodes`, `columns`, and `dependencies`. `constraints` and
  `platform-info` are optional definitions; MSSQL server metadata supplies
  platform context when `platform-info` is unavailable.
