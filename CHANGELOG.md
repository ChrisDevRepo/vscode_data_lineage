...
### Changed
- **Richer enrich_view descriptions — two-layer preservation contract** — the AI's synthesis reminder now states a numeric target for description section length (≥ 50% standalone, ≥ 30% for consolidated variant-sibling sections) AND a two-layer preservation contract. Layer 1 = content that directly answers the user's question (formulas, column meanings, transformation logic, routing predicates) — the AI may format, sequence, and explain the reasoning, but not compress the semantic content. Layer 2 = analyst-noticed enrichment the user should see even if not explicitly asked for (⚠️ risks, silent-failure modes, edge cases, anti-patterns, schema gotchas) — lift the full set when mission-relevant. Addresses observed compression from 90k chars of in-session detail memory down to ~28k chars of rendered description (~69% drop) — sibling consolidation is legitimate but per-slot paraphrase and "keep-only-the-most-dramatic-risk" pruning were eroding the business answer.
